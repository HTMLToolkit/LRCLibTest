(function() {
      ...
      container.querySelectorAll('. result-card').forEach(card => {
        card.addEventListener('click', async () => {
          ...
          try {
            ...
            showError(container, err.data?.message || `Error fetching lyrics (${err.status})`);
          }
        });
      });
    }
    ...
    $('#form-get-signature').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (err.status === 404) {
          ...
          showError(container, err.data?.message || `Error (${err.status})`);
        }
      }
    });
    ...
    $('#btn-challenge').addEventListener('click', async () => {
      ...
      try {
        ...
        status.textContent = 'Failed to get challenge:  ' + (err.data?.message || err.status);
      }
    });
    ...
    $('#form-publish').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        showError(container, err.data?.message || `Publish failed (${err.status})`);
      }
    });
  
    // Init
    console.log('LRCLIB Frontend loaded');
  })();